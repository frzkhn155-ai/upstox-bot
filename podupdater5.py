...
# Known team member first names / short names, used as an additional confirmation check
# alongside the Received At date match. Matching now also accepts substring matches,
# so e.g. "RAGU" will match "Ragupathi Rajabomannan".
TEAM_NAMES = [
    "INFANT", "DINESH", "DURGA", "ELUMALAI", "FEROZ", "PRIYA",
    "SATISH", "SNEHA", "SOWMIYA", "SRINATH", "THIRU", "LOKESH", "PREETI",
    # Add any other short names you use, e.g. "RAGU" if not already present
]
...
def is_team_member(updated_by_text, max_distance=2):
    ...
    Now also accepts substring matches (e.g. 'RAGU' in 'RAGUPATHI').
    Returns (True, matched_team_name) or (False, None).
    """
    if not updated_by_text:
        return False, None
    # Drop trailing employee code like "(CB99354)" and strip
    full_name_upper = updated_by_text.split('(')[0].strip().upper()

    # 1) Substring match (case‑insensitive) – catches short names inside full names
    for team_name in TEAM_NAMES:
        if team_name in full_name_upper or full_name_upper in team_name:
            return True, team_name

    # 2) Original word‑by‑word fuzzy matching (for other cases)
    words = full_name_upper.split()
    for word in words:
        for team_name in TEAM_NAMES:
