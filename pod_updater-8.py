            ...
            if cells:
                ...
                if pod_matches(pod_no, dms_pod):
                    ...
                    try:
                        ...
                        # Mark YES purely on whether the Updated By name matches a
                        # known team member (date is not checked).
                        try:
                            if len(cells) > 6:
                                ...
                                if member_ok:
                                    print(f"[Worker-{worker_id}] [{loan_no}] POD {dms_pod} - Updated by '{updated_by_text}' matches team member '{matched_team_name}' - marking as YES")
                                    status_result = "YES"
                                    updater_name = updated_by_text if updated_by_text else "(name blank)"
                                else:
                                    print(f"[Worker-{worker_id}] [{loan_no}] POD {dms_pod} - Updater '{updated_by_text}' not recognized as team member - marking as NO")
                                    status_result = "NO"
                            else:
                                print(f"[Worker-{worker_id}] [{loan_no}] POD {dms_pod} - Cannot verify Updated By column")
                                status_result = "NO"
