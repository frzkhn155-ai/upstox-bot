...
def pod_matches(input_pod, dms_pod, min_match_length=8):
    """ Flexible POD matching that handles extra/missing digits anywhere. Returns True if there's significant overlap between the two POD numbers. """
            ...
            if cells:
                ...
                if pod_matches(pod_no, dms_pod):
                    ...
                    try:
                        ...
                        # Update link not found - means someone already updated it.
                        # Mark YES regardless of who it was, and note their name.
                        try:
                            if len(cells) > 6:
                                updated_by_text = cells[6].text.strip()
                                
                                if updated_by_text:
                                    print(f"[Worker-{worker_id}] [{loan_no}] POD {dms_pod} already updated by '{updated_by_text}' - marking as YES")
                                    status_result = "YES"
                                    updater_name = updated_by_text
                                else:
                                    print(f"[Worker-{worker_id}] [{loan_no}] POD {dms_pod} - Update link not available, updater name blank")
                                    status_result = "NO"
                            else:
