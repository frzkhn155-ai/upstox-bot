...
# ==== CONFIGURATION ====
NUM_WORKERS = 4  # minimum 1; increase if your PC and site can handle more
            ...
            if cells:
                ...
                if pod_matches(pod_no, dms_pod):
                    ...
                    try:
                        ...
                        # Update link not found - check if already updated by Elumalai
                        try:
                            if len(cells) > 6:
                                updated_by_text = cells[6].text.strip()
                                
                                if "Elumalai" in updated_by_text or "cb50520" in updated_by_text or "Elumalai" in updated_by_text:
                                    print(f"[Worker-{worker_id}] [{loan_no}] POD {dms_pod} already updated by Elumalai - marking as YES")
                                    status_result = "YES"
                                else:
...
def worker_function(records_batch, worker_id):
    ...
    try:
        ...
        driver.find_element(By.ID, "password").send_keys("Hindi@2026")
