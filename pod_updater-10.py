...
# ==== IMMEDIATE STATUS WRITE ====
def write_status(loan_no, status, name):
    """
    Append one result line to status.txt the moment a record finishes
    processing, instead of waiting for every worker to complete. This way
    progress is visible live and nothing is lost if the script is
    interrupted partway through a run.
    """
    with lock:
        with open(status_file, 'a', encoding='utf-8') as sf:
            sf.write(f"{loan_no}\t{status}\t{name}\n")
...
# ==== FLEXIBLE DATE PARSING (for real date comparison, not string matching) ====
_MONTH_NAMES = {
    'jan': 1, 'january': 1,
    'feb': 2, 'february': 2,
    'mar': 3, 'march': 3,
...
def parse_date_flexible(text):
    """
    Parse a date string like '3/7/2026', '03.07.2026', '3-7-26',
    '20-jul-26', '20 July 2026', etc. into a (day, month, year) tuple of
    ints. Handles '/', '.', '-', and space separators, missing leading
    zeros, and text month names (case-insensitive, abbreviated or full).
    Returns None if the text is blank, '-', or not parseable as a date.
    """
    text = (text or "").strip()
    if not text or text == "-":
        return None
        ...
        if sep in text:
            ...
            if len(parts) == 3:
                try:
                    ...
                    month = _MONTH_NAMES.get(month_raw)
                    if month is None:
                        month = int(month_raw)
                    year = int(parts[2])
                    if year < 100:
...
def process_loan(rec, worker_id, driver):
    ...
    try:
        ...
        # Simple retry loop (3 attempts) with exponential backoff (2s, 4s, 8s)
        # for the page load + dispatch tab click - the step most likely to
        # hit transient network/site hiccups.
        dispatch_tab = None
        last_ex = None
        max_retries = 3
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    ...
                    print(f"[Worker-{worker_id}] [{loan_no}] Dispatch tab attempt {attempt}/{max_retries} failed: {ex} - retrying in {backoff}s...")
                    time.sleep(backoff)
        
        if last_ex is not None:
            ex = last_ex
            print(f"[Worker-{worker_id}] [{loan_no}] Dispatch tab not found after {max_retries} attempts: {ex}")
        ...
        for row in table_rows:
            ...
            if cells:
                ...
                if pod_matches(pod_no, dms_pod):
                    ...
                    # Parse date_txt (from 1.txt) into a real (day, month,
                    # year) tuple - handles '.', '-', '/' and missing
                    # leading zeros, so comparisons later are exact, not
                    # string-fragile.
                    date_parsed = parse_date_flexible(date_txt)
                    if date_parsed is None:
                        raise ValueError(f"Unrecognized date format: {date_txt}")
                    date_formatted = f"{date_parsed[0]:02d}/{date_parsed[1]:02d}/{date_parsed[2]}"
                    
                    # First, just check whether an Update button/link exists
                    # on this row at all.
                    try:
                    ...
                    if update_link is None:
                        # No Update button present at all - this row has
                        # already been actioned - mark YES on that basis
                        # alone (no name or date check).
                        print(f"[Worker-{worker_id}] [{loan_no}] POD {dms_pod} - No Update button found - treating as already updated - marking as YES")
                        status_result = "YES"
                        updater_name = cells[6].text.strip() if len(cells) > 6 else ""
                    else:
                        # Button exists - actually try to click it and submit
                        # the form. Only mark YES if this genuinely completes;
                        # any failure partway through means nothing was
                        # submitted, so it must be marked NO, not YES.
                        try:
                            ...
                            # Button was found but the click/select/fill/submit
                            # sequence failed partway - nothing was actually
                            # submitted, so this must be NO, not YES.
                            print(f"[Worker-{worker_id}] [{loan_no}] POD {dms_pod} - Update button found but form submission failed: {type(e).__name__}: {e} - marking as NO")
                            status_result = "NO"
...
if __name__ == "__main__":
    ...
    # Fresh status.txt for this run - write_status() appends to it live as
    # each record finishes, so it must start empty here.
    open(status_file, 'w', encoding='utf-8').close()
    ...
    # status.txt was already written live, record-by-record, via write_status().
    # (Note: with NUM_WORKERS > 1, line order in status.txt reflects completion
    # order across workers, not original 1.txt order.)
